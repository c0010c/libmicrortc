...
DEPS_DIR="$ROOT_DIR/build/fanout-memory/deps/libsrtp-minimal"
VERSION="2.4.2"
URL="http://archive.ubuntu.com/ubuntu/pool/universe/libs/libsrtp2/libsrtp2_${VERSION}.orig.tar.gz"

usage() {
    cat <<'USAGE'
Usage: examples/fanout-memory/build-libsrtp-minimal.sh [options]

Build a fanout-memory-local libsrtp using libsrtp's internal AES/HMAC crypto.
The install is written under build/fanout-memory/deps/libsrtp-minimal/install
and is picked up automatically by examples/fanout-memory/CMakeLists.txt.
...
printf 'Built minimal libsrtp in %s\n' "$INSTALL_DIR"
