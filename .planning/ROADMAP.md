...
| 4 | DTLS-SRTP 安全传输 | 建立安全 backend vtable，完成 DTLS fingerprint、key export 和 SRTP/SRTCP 保护 | SEC-01, SEC-02, SEC-03, SEC-04, SEC-05 | 已完成 |
...
| 6 | Chrome 端到端验收 | 完成测试、Chrome 页面、信令示例和 1v1 音视频通话验收 | TST-01, EXM-01, EXM-02, ACC-01 | 06-01..06-10 已执行，ACC-01 受 secure full E2E 环境和人工媒体 gate 阻断 |
...
**状态：** 已完成。2026-05-10 阶段级 UAT 通过，见 `04-UAT.md`；验证范围为 deterministic backend、核心安全契约和文档收口，Chrome 真实 DTLS/SRTP 端到端验收仍属于第 6 阶段。
...
**状态：** 已完成。2026-05-11 阶段级验证通过，见 `05-VERIFICATION.md`；代码复审 blocker 已修复，剩余异步 executor 语义为非阻塞 advisory。
...
**Wave 5** *(blocked on Wave 4 completion)*
- `05-05-PLAN.md`：PLI 显式请求、远端 PLI/NACK 上报和 NACK no-retransmit 可观测性。已完成，见 `05-05-SUMMARY.md`。

**Wave 6** *(blocked on Wave 5 completion)*
- `05-06-PLAN.md`：媒体 API 文档、UAT、需求追踪和项目状态收口。已完成，见 `05-06-SUMMARY.md`。
...
**状态：** 06-01..06-10 已执行。`06-01` 已完成 Chrome 合成媒体页面、WebSocket 信令服务和 page smoke；`06-02` 已完成 C 示例运行时、WebSocket/UDP 示例层 I/O、PeerConnection observer 和 JSONL summary；`06-03` 已完成样本解析、按节奏发送、接收媒体落盘和 media-file smoke；`06-04` 已完成可选真实安全 backend gate、许可证检查和安全失败分层；`06-05` 已完成端到端自动化编排、compact JSON summary 和七层失败诊断；`06-06` 已完成中文文档、`06-UAT.md`、需求追踪和状态收口，见 `06-06-SUMMARY.md`；`06-07` 已完成共享 runId、C 示例 `--run-id` 和 WebSocket 分片读取状态机，见 `06-07-SUMMARY.md`；`06-08` 已完成 C runtime 成功状态机、media executor 亲和修复、full E2E pass 判定和 smoke 脚本入口，见 `06-08-SUMMARY.md`；`06-09` 已完成默认关闭的 OpenSSL DTLS / libsrtp 可选 security backend，见 `06-09-SUMMARY.md`；`06-10` 已执行默认 OFF gate、dry-run、security-gate 和 secure ON 配置尝试，见 `06-10-SUMMARY.md`。本机缺少可选 OpenSSL/libsrtp 开发依赖，secure full E2E 与 VLC/ffplay 人工播放未完成，`ACC-01` 仍未关闭。
...
**Wave 2** *(blocked on Wave 1 completion)*
- `06-02-PLAN.md`：C 示例运行时、WebSocket client、UDP socket pump、PeerConnection observer 和 JSONL summary。已完成，见 `06-02-SUMMARY.md`。

**Wave 3** *(blocked on Wave 2 completion)*
- `06-03-PLAN.md`：`sample1.opus` / `test-25fps.h264` 样本解析、按节奏发送、接收媒体落盘和 media-file smoke。已完成，见 `06-03-SUMMARY.md`。

**Wave 4** *(blocked on Wave 2 completion)*
- `06-04-PLAN.md`：真实 Chrome DTLS/SRTP 可选安全 backend gate、许可证检查和安全失败分层。已完成，见 `06-04-SUMMARY.md`。

**Wave 5** *(blocked on Wave 1-4 completion)*
- `06-05-PLAN.md`：端到端自动化编排、页面断言、JSONL summary 解析和 signaling/ICE/DTLS/SRTP/RTP/RTCP/media_file 失败分层。已完成，见 `06-05-SUMMARY.md`。

**Wave 6** *(blocked on Wave 5 completion)*
- `06-06-PLAN.md`：中文文档、VLC 人工 UAT、需求追踪、项目状态和路线图收口。已完成，见 `06-06-SUMMARY.md`。

**Wave 7** *(gap closure, blocked on 06-06 verification gaps)*
- `06-07-PLAN.md`：共享 runId、C 示例 `--run-id` 和 WebSocket 分片读取状态机，修复页面与 C 示例信令互达。已完成，见 `06-07-SUMMARY.md`。

**Wave 8** *(blocked on Wave 7 completion)*
- `06-08-PLAN.md`：C runtime 成功状态机、media executor 亲和修复、full E2E pass 判定和 smoke 脚本入口。已完成，见 `06-08-SUMMARY.md`。

**Wave 9** *(blocked on Wave 8 completion)*
- `06-09-PLAN.md`：实现默认关闭的真实可选 OpenSSL/libsrtp DTLS/SRTP backend，保持默认构建无系统安全依赖。
  已完成，见 `06-09-SUMMARY.md`。

**Wave 10** *(blocked on Wave 9 completion)*
- `06-10-PLAN.md`：secure full E2E、VLC/ffplay 人工播放批准、需求追踪和项目状态最终收口。
  已执行，见 `06-10-SUMMARY.md`；本机缺少可选 OpenSSL/libsrtp 开发依赖，secure full E2E 与 VLC/ffplay 人工播放未完成。
