# v1.0 里程碑路线图归档：WebRTC 纯 C 库

**状态：** 已发货，2026-05-11
**阶段：** 1-6
**计划：** 35/35 完成
**需求：** v1 48/48 完成
**说明：** 本文件保留 v1.0 关闭时的完整 `ROADMAP.md` 内容，当前路线图请见 `.planning/ROADMAP.md`。

## 归档备注

- 产品目标已达成：纯 C、固定内存、无线程、用户负责 UDP/socket 收发边界下，完成与 Chrome 的 1v1 音视频 `PeerConnection` 互通。
- secure full E2E 已通过，输出 `pass:true`、`layer:"none"`，并生成非空 H264/Opus 媒体产物。
- 用户已于 2026-05-11 批准 VLC/ffplay 人工媒体播放 gate，`ACC-01` 关闭。
- 已知审计技术债：`.planning/milestones/v1.0-MILESTONE-AUDIT.md` 状态为 `gaps_found`，主要因为第 1、3、4 阶段缺少严格工作流要求的 `*-VERIFICATION.md`；实现链路和端到端验收未发现产品级阻断。
...
| 3 | ICE/STUN 与 Datagram 网络层 | 实现 host/srflx、Full ICE、trickle ICE、状态事件和 datagram demux | SDP-05, ICE-01, ICE-02, ICE-03, ICE-04, ICE-05, NET-01, NET-02, NET-03 | 已完成 |
...
| 6 | Chrome 端到端验收 | 完成测试、Chrome 页面、信令示例和 1v1 音视频通话验收 | TST-01, EXM-01, EXM-02, ACC-01 | 已完成 |
...
**状态：** 已完成。2026-05-10 阶段级 UAT 通过，见 `03-UAT.md`；验证范围为固定容量 ICE/STUN、host/srflx gathering、trickle candidate、Full ICE checks、regular nomination、失败原因可观测性、datagram demux 和中文 API 文档，Chrome 真实端到端验收仍属于第 6 阶段。
...
**Wave 3** *(blocked on Wave 2 completion)*
- `06-03-PLAN.md`：`sample1.opus` / `chrome-25fps-42001f.h264` 样本解析、按节奏发送、接收媒体落盘和 media-file smoke。已完成，见 `06-03-SUMMARY.md`。
...
- `06-10-PLAN.md`：secure full E2E、VLC/ffplay 人工播放批准、需求追踪和项目状态最终收口。
  已执行，见 `06-10-SUMMARY.md`；后续 quick task 已解除本机可选 OpenSSL/libsrtp 缺依赖 blocker。

**Wave 11** *(gap closure, blocked on 06-10 UAT diagnosis)*
- `06-11-PLAN.md`：Chrome ICE/STUN authenticated connectivity checks、incoming Binding success response、secure binary selection 和 full E2E 重跑。已完成，见 `06-11-SUMMARY.md`；secure full E2E 已通过，VLC/ffplay 人工播放 gate 已由用户批准。
...
---
*最后更新：2026-05-11，Phase 6 secure full E2E ICE gap 诊断后*
