...
| `reflib/kvs-webrtc-sdk/samples/p2p/kvsWebRTCClientMaster.c` | `examples/chrome-e2e/mrtc_chrome_answerer.c` | `9eebcc4` | `reference-only` | Phase 6 Chrome answerer demo 只参考 KVS sample 的 Answerer/master 使用边界和固定媒体输入概念；实现为原创 JSON-line demo，不搬入 KVS signaling、credential 或 GStreamer sample 代码。 | `supporting/reference-only` | `E2E-01, E2E-02, E2E-03, E2E-07, E2E-08` | `06` | `draft` |
| `reflib/kvs-webrtc-sdk/samples/p2p/kvsWebRTCClientMaster.c` | `examples/chrome-e2e/browser-client.js` | `9eebcc4` | `reference-only` | Phase 6 browser client 的 SDP/candidate/DataChannel/media 控制逻辑为原创 Playwright-facing harness；仅以 KVS sample 的角色划分和媒体互通目标为参考。 | `supporting/reference-only` | `E2E-01, E2E-03, E2E-04, E2E-05, E2E-06` | `06` | `draft` |
| `reflib/kvs-webrtc-sdk/tst/PeerConnectionFunctionalityTest.cpp` | `tests/e2e/chrome-host.spec.js` | `9eebcc4` | `reference-only` | Phase 6 host Chrome E2E 为原创 Playwright spec；参考本地 KVS peer connection 功能测试的连接/DataChannel/媒体验收分层，不复制 C++ 测试代码。 | `supporting/reference-only` | `E2E-03, E2E-04, E2E-05, E2E-06, E2E-07, E2E-08, TEST-02` | `06` | `draft` |
| `reflib/kvs-webrtc-sdk/tst/TurnConnectionFunctionalityTest.cpp` | `tests/e2e/chrome-turn.spec.js` | `9eebcc4` | `reference-only` | Phase 6 TURN relay E2E 为原创 Playwright spec；参考本地 KVS TURN relay 验收边界，强制 relay-only 并断言 selected candidate pair 为 relay。 | `supporting/reference-only` | `NET-04, E2E-04, E2E-05, E2E-06, E2E-07, E2E-08, TEST-02` | `06` | `draft` |
| `reflib/kvs-webrtc-sdk/tst/` | `tests/e2e/summary.js` | `9eebcc4` | `reference-only` | Phase 6 summary helper 为原创 Node 工具；参考 KVS 测试按阶段定位失败的验收方式，输出 MRTC 专用 JSON schema 并屏蔽 TURN secret。 | `supporting/reference-only` | `TEST-03, TEST-04, NET-04` | `06` | `draft` |
| `reflib/kvs-webrtc-sdk/tst/CMakeLists.txt` | `scripts/verify-v1.sh` | `9eebcc4` | `reference-only` | Phase 6 总验收脚本为原创 shell 入口；参考 KVS 测试/构建收口需要一条可复现命令的边界，串起 CMake、CTest、host E2E 和显式 TURN E2E。 | `supporting/reference-only` | `TEST-01, TEST-02, TEST-03, TEST-04, NET-04` | `06` | `draft` |
