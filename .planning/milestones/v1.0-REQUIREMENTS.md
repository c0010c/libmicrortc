# v1.0 里程碑需求归档：WebRTC 纯 C 库

**状态：** 已发货，2026-05-11
**需求完成：** v1 48/48 已完成
**说明：** 本文件保留 v1.0 关闭时的完整 `REQUIREMENTS.md` 内容。关闭后 live `.planning/REQUIREMENTS.md` 将删除，下一里程碑由 `$gsd-new-milestone` 重新定义需求。

## 需求结果

- v1 公共 API、固定内存、执行器、SDP/JSEP、ICE/STUN、DTLS-SRTP、RTP/RTCP、可观测性、构建、示例和验收需求全部标记完成。
- v2 网络、媒体、DataChannel、安全 backend 和 trace 工具增强保留为后续候选方向。
- 已知审计技术债：严格审计要求第 1、3、4 阶段补齐 `*-VERIFICATION.md`；该 gap 已由用户选择作为技术债接受，不阻断 v1.0 归档。
...
- [x] **SDP-05**: 库支持 trickle ICE 所需的本地候选输出和远端候选输入。

### ICE 与网络 datagram

- [x] **ICE-01**: 库实现 Full ICE connectivity checks，支持 controlling 和 controlled 角色。
- [x] **ICE-02**: 库支持 host candidate。
- [x] **ICE-03**: 库通过 STUN 支持 server reflexive candidate。
- [x] **ICE-04**: 库支持 trickle ICE candidate 的增量处理。
- [x] **ICE-05**: 库通过 observer 输出 ICE gathering、checking、connected、failed 等状态事件。
- [x] **NET-01**: 用户可以把收到的 UDP datagram 输入给库，库完成 STUN、DTLS、RTP、RTCP 分类。
- [x] **NET-02**: 库通过回调输出待发送 UDP datagram，由用户负责 socket 发送。
- [x] **NET-03**: datagram 输入输出 API 明确 buffer 所有权和生命周期。
...
- [x] **ACC-01**: 用户可以通过本地 Chrome 页面和信令示例完成 1v1 音视频通话验收。
...
| SDP-04 | 第 2 阶段 | 已完成 |
| API-04 | 第 2 阶段 | 已完成 |
| SDP-05 | 第 3 阶段 | 已完成：03-01/03-04 支持本地 candidate 输出和远端 `candidate:`/`a=candidate:` 输入；`03-UAT.md` 阶段级验证通过 |
| ICE-01 | 第 3 阶段 | 已完成：03-03 实现 Full ICE checks、controlling/controlled 角色、regular nomination、selected pair trace；`03-UAT.md` 阶段级验证通过 |
| ICE-02 | 第 3 阶段 | 已完成：03-02 实现 host candidate gathering 与输出；`03-UAT.md` 阶段级验证通过 |
| ICE-03 | 第 3 阶段 | 已完成：03-02 实现 STUN Binding request、IPv4 `XOR-MAPPED-ADDRESS` response 和 srflx candidate 生成；`03-UAT.md` 阶段级验证通过 |
| ICE-04 | 第 3 阶段 | 已完成：03-01/03-03 支持 trickle candidate 增量输入和 pair 增量处理；`03-UAT.md` 阶段级验证通过 |
| ICE-05 | 第 3 阶段 | 已完成：03-02/03-03/03-04 输出 gathering、checking、connected、failed 状态及失败原因 trace/counter；`03-UAT.md` 阶段级验证通过 |
| NET-01 | 第 3 阶段 | 已完成：03-04 分类 STUN、DTLS、RTP、RTCP 和 unknown datagram，STUN 路由到 ICE/STUN handler；`03-UAT.md` 阶段级验证通过 |
| NET-02 | 第 3 阶段 | 已完成：03-02/03-03 通过 `observer.on_datagram` 输出 STUN gathering/check datagram，由用户负责 socket 发送；`03-UAT.md` 阶段级验证通过 |
| NET-03 | 第 3 阶段 | 已完成：03-01/03-04 和中文 API 文档明确 candidate/datagram buffer 生命周期、固定容量和用户 socket 边界；`03-UAT.md` 阶段级验证通过 |
...
| ACC-01 | 第 6 阶段 | 已完成：06-02 已完成 C 示例运行时、WebSocket/UDP 示例层 I/O 和 JSONL observer；06-03 已完成样本解析、按节奏发送、接收媒体落盘和 media-file smoke；06-04 已完成默认关闭的可选真实安全 backend gate、许可证说明和 DTLS/SRTP/RTP/RTCP 失败分层；06-05 已完成 full E2E 自动化编排和 `manual_vlc_required:true` 报告；06-07 修复共享 `runId` 和 WebSocket 分片读取；06-08 修复 C runtime 成功状态机、media executor 亲和和 full E2E pass 判定；06-09 实现默认关闭的 OpenSSL DTLS/libsrtp 可选 backend；06-10 已执行默认 OFF gate、dry-run、security-gate 和 secure ON 配置尝试；20260511 quick task 已在当前机器安装用户级 OpenSSL 1.1.1w/libsrtp 2.6.0 并验证 secure ON 配置和 `rtc_chrome_e2e` 构建通过；06-11 已修复 Chrome ICE/STUN authenticated Binding request/response、peer-reflexive 示例层候选补齐和 secure binary selection；secure full E2E 已输出 `pass:true`、`layer:"none"` 并生成非空 `received-h264.264`/`received-opus.packets`；2026-05-11 用户已在 `$gsd-verify-work 6` Test 8 中批准 VLC/ffplay 人工媒体播放 gate，`ACC-01` 关闭 |
...
---
*需求定义：2026-05-10*
*最后更新：2026-05-11，第 6 阶段 UAT 完成并关闭 ACC-01 后*
