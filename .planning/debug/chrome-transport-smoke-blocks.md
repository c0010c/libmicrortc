...
- **fix:** 已修复一个 smoke 假阳性风险：browser-client 现在把 answerer 诊断事件记录为 `remote.*`，避免远端 `datachannel.message` 或 `pc.connected` 被当作浏览器本地事件。真实 transport 互通未在本次 debug 中完成，需要单独实现 ICE/DTLS/SCTP packet path。
- **verification:** `MRTC_E2E_BROWSER_CHANNEL=chromium npm --prefix tests/e2e run test:host -- --grep "transport smoke"` 通过分类模式；严格 `MRTC_E2E_REQUIRE_TRANSPORT=1` 仍按预期失败在 connection stage。
- **files_changed:** `examples/chrome-e2e/browser-client.js`, `.planning/debug/chrome-transport-smoke-blocks.md`
