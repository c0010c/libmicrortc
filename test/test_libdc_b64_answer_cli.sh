...
if [[ $# -gt 1 ]]; then
  echo "usage: $0 [<external_client_build_dir>]" >&2
  exit 2
fi

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ext_dir="$repo_root/external-client"
ext_build_dir="${1:-$ext_dir/build}"

if [[ ! -f "$ext_dir/CMakeLists.txt" ]]; then
  echo "[libdc-b64] missing external-client project: $ext_dir" >&2
  exit 2
fi

echo "[libdc-b64] configuring external-client"
cmake -S "$ext_dir" -B "$ext_build_dir" -DCMAKE_BUILD_TYPE=Release >/dev/null

echo "[libdc-b64] building libdc_b64_answer_cli"
cmake --build "$ext_build_dir" --target libdc_b64_answer_cli >/dev/null

bin="$ext_build_dir/libdc_b64_answer_cli"
if [[ ! -x "$bin" ]]; then
  echo "[libdc-b64] missing binary: $bin" >&2
  exit 1
fi
...
if "$bin" --run-ms 50 </dev/null >"$tmp_out" 2>"$tmp_err"; then
  echo "[libdc-b64] expected failure for empty offer" >&2
  exit 1
fi
grep -Eq 'offer|decode|failed' "$tmp_out"

if echo '%%%INVALID%%%' | "$bin" --run-ms 50 >"$tmp_out" 2>"$tmp_err"; then
  echo "[libdc-b64] expected failure for invalid base64 chars" >&2
  exit 1
fi
grep -Eq 'offer|decode|failed' "$tmp_out"

if printf 'A===' | "$bin" --run-ms 50 >"$tmp_out" 2>"$tmp_err"; then
  echo "[libdc-b64] expected failure for invalid base64 padding" >&2
  exit 1
fi
...
if "$bin" --run-ms 50 <"$tmp_big_raw" >"$tmp_out" 2>"$tmp_err"; then
  echo "[libdc-b64] expected failure for oversized input" >&2
  exit 1
fi
grep -Eq 'offer|decode|failed' "$tmp_out"
...
if ! "$bin" --run-ms 120 <"$tmp_offer_b64" >"$tmp_out" 2>"$tmp_err"; then
  echo "[libdc-b64] expected success for long offer" >&2
  cat "$tmp_out" >&2 || true
  cat "$tmp_err" >&2 || true
  exit 1
fi
